...
function LiveSpellInstance($setup) {
    ...
    this.CheckInWindow = function () {
        ...
        if (wm.substr(0, 9) == "jquery.ui" && (typeof jQuery === "undefined" ? "undefined" : _typeof(jQuery)) && _typeof($.ui)) {
        ...
        if (wm == "fancybox" && (typeof jQuery === "undefined" ? "undefined" : _typeof(jQuery))) {
            var uri = this.url();
            var ttl = livespell.lang.fetch(this.id(), "WIN_TITLE");
            $.fancybox({
                width: Dwidth,
...
if (typeof livespell == "undefined") {
    var livespell = {
        ...
        setrubberRingServerModel: function setrubberRingServerModel() {
        ...
        setUniServerModel: undefined.setrubberRingServerModel,
        ajaxClient: function ajaxClient(sync) {
            var xhr = false;
            try {
        ...
        testSyncRequest: function testSyncRequest(posturl) {
            var hasajax = false;
        ...
        getIframeDocumentBasic: function getIframeDocumentBasic(myFrame) {
            var oDoc;
            var oFrameWin;
        ...
        getIframeDocument: function getIframeDocument(myFrame) {
            var oDoc, oBody;
            var isEditable;
            try {
                if (myFrame.contentWindow) {
        ...
        lang: {
            fetch: function fetch(providerID, index) {
        ...
        ajax: {
            renderIframe: function renderIframe(postURL) {
            ...
            sendcount: 0,
            send: function send(cmd, args, lan, note, sender) {
                this.sendcount++;
                ...
                if (hasajax) {
                    var liveSpellAjaxCallbackhandler = function liveSpellAjaxCallbackhandler() {
                        if (xhr.readyState == 4) {
                            if (xhr.status == 200) {
            ...
            send_sync: function send_sync(cmd, args, lan, note, sender) {
                livespell.cache.ajaxrequest = {};
                livespell.cache.ajaxrequest.cmd = cmd;
                livespell.cache.ajaxrequest.args = args;
                livespell.cache.ajaxrequest.lan = lan;
            ...
            pickupIframe: function pickupIframe(strHTML) {
                strHTML = strHTML.split("<script")[0];
                strHTML = strHTML.split("<SCRIPT")[0];
                this.pickup(strHTML, false);
            },
            debug: function debug(msg, mtcolor) {
                if (typeof window['LIVESPELL_DEBUG_MODE'] != "undefined") {
            ...
            pickup: function pickup(strHTML, binInstant) {
        ...
        test: {
            ...
            BuggyAjaxInFireFox: function BuggyAjaxInFireFox() {
            ...
            ALLCAPS: function ALLCAPS(str) {
                return str === str.toUpperCase();
            },
            eos: function eos(str) {
                str = str.replace(/&nbsp;/gi, " ");
            ...
            browserNoAYT: function browserNoAYT() {
            ...
            spelling: function spelling(word, Lang) {
            ...
            fullyCached: function fullyCached(word, lang, makeSuggestions) {
        ...
        str: {
            ...
            stripSpans: function stripSpans(strinput) {
                if (!strinput) {
                    return "";
                }
            ...
            htmlDecode: function htmlDecode(content) {
                if (livespell.test.IE5() || livespell.test.IE6() || livespell.test.IE7() || livespell.test.IE8()) {
                    var EntityLevelHtmlDecode = function EntityLevelHtmlDecode(input) {
            ...
            stripTags: function stripTags(strinput) {
                if (!strinput) {
                    return "";
                }
            ...
            stripComments: function stripComments(strinput) {
                if (!strinput) {
                    return "";
                }
            ...
            HTMLDec: function HTMLDec(s) {
                s = s = s.replace(/\&nbsp\;/gi, " ");
                s = s.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
                return s;
            },
            spliceXHTML: function spliceXHTML(str, pos, add) {
            ...
            spliceSpans: function spliceSpans(str, pos, add) {
                if (pos == -1) {
                    return str;
                }
                if (pos == 0) {
            ...
            toCase: function toCase($$str, $$C, $$bcapitalize) {
                switch ($$C) {
                    case 2:
                        $$str = $$str.toUpperCase();
                        break;
            ...
            tokenize: function tokenize(strdoc, binraw) {
        ...
        cookie: {
            erase: function erase(name, path, domain) {
                this.setLocal(name, "");
            },
            get: function get(check_name) {
        ...
        events: {
            add: function add(obj, event, callback, capture) {
        ...
        array: {
            safepush: function safepush(arr, value) {
            ...
            remove: function remove(array, subject) {
                var r = new Array();
    ...
    livespell.insitu = {
        ...
        updateBaseDelay: function updateBaseDelay(id) {
            var id2 = id;
            var n = function n() {
        ...
        updateBase: function updateBase(id) {
            E$(id).value = livespell.insitu.getProxyText(id);
        },
        proxyDOM: function proxyDOM(id) {
            return E$(id + this._FIELDSUFFIX);
        },
        getProxyHTML: function getProxyHTML(id) {
            return livespell.insitu.proxyDOM(id).innerHTML;
        },
        extractTextWithWhitespace: function extractTextWithWhitespace(parent) {
            var ret = "";
        ...
        getProxyText: function getProxyText(id) {
        ...
        setProxyHTML: function setProxyHTML(id, val) {
            livespell.insitu.proxyDOM(id).innerHTML = val;
            livespell.context.validate(id);
        },
        setProxyText: function setProxyText(id, val) {
        ...
        safeUpdateProxy: function safeUpdateProxy(id, providerId) {
        ...
        updateProxy: function updateProxy(id) {
            var value = E$(id).value;
            this.setProxyText(id, value);
        },
        setProxyHTMLAndMaintainCaretLegacy: function setProxyHTMLAndMaintainCaretLegacy(id, val) {
            var e;
            var dmilit = "\uFEFF";
        ...
        setProxyHTMLAndMaintainCaret: function setProxyHTMLAndMaintainCaret(id, val) {
        ...
        findLiveChildrenInDOMElement: function findLiveChildrenInDOMElement(element) {
            var uid;
            var LiveOffspring = [];
            if (element.id) {
                uid = element.id;
        ...
        checkNow: function checkNow(fieldList, providerID) {
            var txt = '';
            if (window.getSelection) {
        ...
        destroyProxy: function destroyProxy(id) {
        ...
        createProxy: function createProxy(id) {
            ...
            if (livespell.test.FireFox()) {
                ...
                n.onpaste = function () {
                    ...
                    n.innerHTML = "";
                    var ff_onafterpaste = function ff_onafterpaste() {
        ...
        cloneClientEventsHelper: function cloneClientEventsHelper(from, to, sevent) {
            var element = from;
            var proxy = to;
            var event = sevent;
            var fn = function fn() {
        ...
        cloneClientEvents: function cloneClientEvents(from, to) {
        ...
        addCss: function addCss(cssCode) {
            try {
        ...
        renderProxy: function renderProxy(fieldList, providerID) {
            if (!fieldList) {
                return;
            }
            if (!fieldList.join) {
        ...
        idcount: 0,
        filterTextAreas: function filterTextAreas(Area) {
            var ok = true;
        ...
        filterTextInputs: function filterTextInputs(Area) {
        ...
        filerEditors: function filerEditors(elem) {
        ...
        maxCharsHandler: function maxCharsHandler(event) {
        ...
        strictMaxChars: function strictMaxChars(obj) {
        ...
        ignoreAtCursor: function ignoreAtCursor() {
            var target;
            try {
                if (window.getSelection) {
        ...
        disableclick: function disableclick(event, providerID) {
        ...
        typoclick: function typoclick(event, oparent, ospan, reason, providerID) {
            var prov = livespell.spellingProviders[providerID];
            if (prov && prov.RightClickOnly && event.button != 2) {
                return true;
            }
    ...
    livespell.context = {
        ...
        keysTrapped: false,
        providerID: null,
        mackeydown: function mackeydown(event) {
            try {
                if (!event) {
                    event = window.event;
                }
        ...
        hideIfNotinUse: function hideIfNotinUse() {
        ...
        notifyBaseFieldOnChange: function notifyBaseFieldOnChange(base_field_id) {
        ...
        changeLanguage: function changeLanguage(strLang) {
        ...
        showMultiLang: function showMultiLang() {
        ...
        hideMultiLang: function hideMultiLang() {
            E$('livepell__multilanguage').style.display = "none";
        },
        base_field_id: function base_field_id() {
        ...
        showMenu: function showMenu(id, word, reason, providerID) {
        ...
        setMultiLang: function setMultiLang() {
        ...
        css_list: Array(),
        renderCss: function renderCss(strTheme) {
        ...
        buttonIsHidden: function buttonIsHidden(strId, providerID) {
            var oProvider = livespell.spellingProviders[providerID];
        ...
        render: function render(suggs, action, providerID, reason) {
            if (livespell.test.iPhone()) {
                return false;
            }
        ...
        boundMenuToScreen: function boundMenuToScreen() {
        ...
        DimViewport: function DimViewport() {
            var w = 0;
            var h = 0;
        ...
        suggest: function suggest(id, word, reason, caller) {
...
if (typeof jQuery != "undefined") {
    (function ($) {
        ...
        var livespell$set = function livespell$set(options) {
